...
- (instancetype)initWithMessage:(QBChatMessage *)chatMessage
{
    NSParameterAssert(chatMessage.text != nil);
    NSParameterAssert(chatMessage.dateSent != nil);
    self = [super init];
    if (self) {
        _senderId = [NSString stringWithFormat:@"%lu",(unsigned long)chatMessage.senderID ];
        _text = [chatMessage.text copy];
        _date = [chatMessage.dateSent copy];
        _isMediaMessage = NO;
        _myMessage = chatMessage.myMessage;
    }
    return self;
}
