...
- (void)setMediaView:(UIView *)mediaView
{
    ...
    [self.messageBubbleContainerView jsq_pinAllEdgesOfPhotoView:mediaView];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        for (NSUInteger i = 0; i < self.messageBubbleContainerView.subviews.count; i++) {
            if (self.messageBubbleContainerView.subviews[i] != _mediaView &&
                self.messageBubbleContainerView.subviews[i] != self.messageBubbleImageView &&
                self.messageBubbleContainerView.subviews[i] != self.cellBottomLabel) {
