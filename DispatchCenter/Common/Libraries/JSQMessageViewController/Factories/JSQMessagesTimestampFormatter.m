...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _timeTextAttributes = @{ NSFontAttributeName : [UIFont systemFontOfSize:8.0f],
                                 NSForegroundColorAttributeName : color,
                                 NSParagraphStyleAttributeName : paragraphStyle };
    }
    return self;
...
- (NSAttributedString *)attributedGroupMessageDate:(NSDate *)date
{
    if (!date) {
        return nil;
    }
...
- (NSString *)timeForDate:(NSDate *)date
{
    ...
    [self.dateFormatter setTimeStyle:NSDateFormatterShortStyle];
    return [self.dateFormatter stringFromDate:date].lowercaseString;
}
...
- (NSString *)relativeDateForDate:(NSDate *)date
{
    ...
    return [date stringValueFormattedBy:@"EEE, MMM dd,yyyy"];
}
