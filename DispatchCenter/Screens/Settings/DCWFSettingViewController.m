...
@interface DCWFSettingViewController()
@end

@implementation DCWFSettingViewController
...
- (void)refreshNotificationStatus {
    ...
    if (APP_DELEGATE.mySettings.online) {
        self.onlineStatusLabel.text = NSLocalizedString(@"On",nil);
        self.onlineStatusLabel.textColor = kDCGlobalYellowColorBase;
        
    } else {
        self.onlineStatusLabel.text = NSLocalizedString(@"Off",nil);
        self.onlineStatusLabel.textColor = kDCGlobalGrayColorBase;
    }
}
...
- (IBAction)settingOnlineStatus:(id)sender {
    [MAIN_NAVIGATION pushViewController:[[DCWFSettingOnlineViewController alloc] initWithNibName:@"DCWFSettingOnlineViewController" bundle:nil] animated:YES];
}
